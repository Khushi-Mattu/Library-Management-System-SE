window.addEventListener("load", function () {
    ...
    Array.from(is_interested_images).forEach(element => {
        element.addEventListener("click", function (event) {
            ...
            var book_id = event.target.getAttribute("book_id");
            ...
            // Set up request
            XHR.open("GET", "api/toggle_interested.php?property_id=" + book_id);
...
var toggle_interested_success = function (event) {
    ...
    if (response.success) {
        ...
        var is_interested_image = document.querySelectorAll(".book-" +book_id + " .is-interested-image")[0];
        var interested_user_count = document.querySelectorAll(".book-id-" + book_id + " .interested-user-count")[0];
