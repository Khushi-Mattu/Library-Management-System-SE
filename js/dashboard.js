window.addEventListener("load", function () {
    ...
    Array.from(is_interested_images).forEach(element => {
        element.addEventListener("click", function (event) {
            ...
            var property_id = event.target.getAttribute("book_id");
            ...
            // Set up request
            XHR.open("GET", "api/toggle_interested.php?book_id=" + book_id);
...
var remove_interested_success = function (event) {
    ...
    if (response.success) {
        ...
        document.getElementsByClassName("property-id-" + book_id)[0].style.display = 'none';
    }
};
